...
df <- read.table("words_as_y_axis.txt",
                 header = F,
                 sep = "\t",
                 quote="\'")

#subset the data to produce sample analysis more swiftly
df <- df[df$V1 %in% c("Tmp.xml"),]

df$custom_order <- factor(df$V7, levels = c("2 Henry VI (1591)",
"3 Henry VI (1591)",
...
ggplot(df, aes(V4, V3, fill=V5)) + 
  ...
  xlab("Play Time (in Words)") + 
  ylab("Characters on Stage") +
  labs(fill="")
